...
export async function GET() {
    try {
        ...
        const { data: movieLinks } = await supabase
            .from('download_links')
            .select('id, mega_link, gdrive_link, mediafire_link, terabox_link, pcloud_link, youtube_link, link_status, last_checked_at')
            .order('last_checked_at', { ascending: true, nullsFirst: true })
            .limit(limit);

        if (movieLinks) {
            for (const row of movieLinks) {
                const linkRow = row as Record<string, any>;
                const statusUpdate: Record<string, any> = { ...(row.link_status || {}) };
                let hasChange = false;

                for (const provider of PROVIDERS) {
                    const link = linkRow[provider.key];
                    if (link) {
