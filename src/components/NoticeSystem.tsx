...
type NoticeFields = Pick<Notice, 'id' | 'type' | 'content' | 'image_url' | 'pages' | 'bg_color' | 'text_color'>;
...
export default function NoticeSystem() {
    ...
    useEffect(() => {
        async function fetchNotices() {
            ...
            // 2. Fetch from Supabase with specific columns
            const { data } = await supabase
                .from('notices')
                .select('id, type, content, image_url, pages, bg_color, text_color')
                .eq('is_active', true);
    ...
    const isMatch = (notice: Pick<Notice, 'id' | 'pages'>) => {
            ...
            {inlineNotices.length > 0 && (
                <div className="relative z-40 container mx-auto px-4 mt-24 mb-6 space-y-4">
                    {inlineNotices.map(notice => (
                        <div
                            key={notice.id}
                            className={`px-6 py-4 rounded-xl shadow-lg border border-white/10 relative font-bold ${isTailwind(notice.bg_color) ? notice.bg_color : ''} ${isTailwind(notice.text_color) ? notice.text_color : ''}`}
                            style={{
                                backgroundColor: !isTailwind(notice.bg_color) ? notice.bg_color : undefined,
                                color: !isTailwind(notice.text_color) ? notice.text_color : undefined
                            }}
                        >
                            <div className={`flex ${notice.image_url ? 'flex-col sm:flex-row items-start gap-4' : 'items-center justify-between'}`}>
                                {notice.image_url && (
                                    <img
                                        src={notice.image_url}
                                        alt=""
                                        className="w-full sm:w-32 h-auto rounded-lg object-cover flex-shrink-0"
                                    />
                                )}
                                <div className="flex-1" dangerouslySetInnerHTML={{ __html: notice.content }} />
                                <button
                                    onClick={() => handleClose(notice.id)}
                                    className="p-2 hover:bg-black/10 rounded-lg transition-colors flex-shrink-0 absolute top-2 right-2 sm:static"
                                >
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
                    ...
                    >
                        <button
                            ...
                            className={`absolute top-2 right-2 p-2 bg-black/20 hover:bg-black/40 rounded-full transition-colors z-10 ${isTailwind(popupNotices[0].text_color) ? popupNotices[0].text_color : ''}`}
                        ...
                        {popupNotices[0].image_url && (
                            <img
                                ...
                                alt=""
                                className="w-full h-auto rounded-xl mb-4 object-cover max-h-64"
                            />
                        )}
